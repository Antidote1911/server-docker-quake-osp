...
if [ -z "$(find /quake/osp -mindepth 1 -maxdepth 1)" ]; then
    pinfo "Folder osp is empty or not exist. Copy from static folder..."
    cp -a "/static/files/cpma/." "/quake/osp/"
    chmod -R 777 /quake/osp/
else
    echo "Folder osp is not empty."
fi
